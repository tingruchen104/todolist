...
>
> **前置需求（Figma）**：動工前先確認 Figma MCP 已連線並授權（`whoami`）；未連線／未授權 → 停止，平實提示登入 Figma 後再試。
>
> **前置需求（瀏覽器）**：量測與截圖比對需瀏覽器，僅開 localhost。優先 `claude-in-chrome`；未連線可退 Playwright；兩者皆無 → 停止，平實提示安裝並登入 Chrome 擴充（各人各自安裝）後再試。
